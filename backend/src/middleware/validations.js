...
const isSelfOrAdmin = (req, res, next) => {
    ...
    if (req.user.role === ROLES.ADMIN || req.user.userId === requestedUserId) {
        return next();
    }
...
const canAccessPair = (req, res, next) => {
    ...
    // Les admins ont accès à toutes les paires
    if (req.user.role === ROLES.ADMIN) {
        return next();
    }
...
const menteeOnly = (req, res, next) => {
    ...
    if (req.user.role !== ROLES.MENTEE && req.user.role !== ROLES.ADMIN) {
        return res.status(403).json({
            error: 'Accès non autorisé',
